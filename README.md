...
- 去重检测（bvid + cid）
- **拖拽排序由封面触发**（封面静态 `draggable`，图片是 HTML5 最可靠的拖拽源）；
  拖拽时封面**不放大**（与悬停预览互斥）；封面悬停 2 秒放大 3 倍预览；
...
v1.8.2 拖拽排序改为拖封面触发（静态 draggable，更可靠）、拖拽时封面不放大、
悬停放大改为 2 秒延迟 3 倍。单元测试共 32 项全部通过。待用户在浏览器验证：
1. **拖住封面上下拖动可排序**，落位准确（拖到哪首歌就落在哪首歌的位置）
2. 拖拽过程中封面**不会**触发悬停放大
3. 封面悬停约 2 秒后放大 3 倍预览，移开即收回
4. 六点已移除；封面单击仍可播放，长按（标题区）进入复选
