...
- 去重检测（bvid + cid）
- **拖拽排序仅由最左侧六点 `⋮⋮` 触发**（其余区域点击为播放/选择，改名框内可正常划选文字）
...
共 30 项断言（background 18 + content 12），全部通过（退出码 0）才算合格。
...
v1.8.0 新增批量复选（移动/复制/删除）与六点拖拽，修复胶囊悬停透明度、图标居中。
单元测试共 30 项（background 18 + content 12）全部通过。待用户在浏览器验证：
1. 长按歌曲进入复选模式，点选多首后可批量移动/复制到其他歌单、批量删除、取消
2. 拖拽排序仅在最左侧六点 `⋮⋮` 上触发；改名框内可正常划选文字
3. 胶囊暂停时半透明、悬停恢复不透明；播放/暂停键与停止键图标居中
