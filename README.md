...
v1.8.1 控制图标全面改用 SVG（精确居中）、修复拖拽落位、移除单项删除按钮、封面悬停放大。
单元测试共 32 项（background 20 + content 12）全部通过。待用户在浏览器验证：
1. 胶囊与面板的 上一首/播放·暂停/下一首/停止 图标居中，悬停放大不再位移
2. 拖拽排序（六点触发）落位准确：拖到哪首歌就落在哪首歌的位置
3. 列表封面悬停约 1 秒后放大预览
4. 每首歌右侧无 ×（删除走长按复选）
