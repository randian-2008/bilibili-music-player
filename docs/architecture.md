...
导入请求的同步边界是“榜单读取、占位条目生成、一次批量写入和数据广播”。它不会等待 B站搜索或音频流验证完成，因此榜单窗口的确定响应不受整张列表匹配时间影响。B站搜索接口返回 HTTP 412 时，搜索层单独进行最多四次请求（初次请求加 1.5、3、6 秒退避）；候选视频不可播放时，外层仍按手动匹配预算重新搜索，最多十轮。两层预算均耗尽后保存失败状态，不会无限请求。
