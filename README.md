...
> 使用与维护入口：[使用指南](USER_GUIDE.md) · [架构说明](docs/architecture.md) · [贡献指南](CONTRIBUTING.md) · [安全政策](SECURITY.md) · [提交 Issue](.github/ISSUE_TEMPLATE)
...
- 补充面向最终用户的独立安装、更新、操作与故障排查指南，并将其纳入最小发布包。
- 采用 GNU GPL v3.0 或更高版本，发布包同步携带完整许可证文本。
- 增加 GitHub Actions、Issue/PR 模板、贡献指南、安全政策和维护者架构文档。
- 清理旧版开发状态说明，统一 Manifest 与项目元数据版本。

### v2.5.0（播放器界面打磨与主题扩展）

- 播放器停止时压缩为仅保留封面、标题和播放入口的紧凑形态；正在播放时保留完整控制区，并延续最近一次有效封面，避免空态裂图。
- 面板支持拖拽调整尺寸并记忆位置；歌单菜单可独立滚动，长菜单不再被播放列表截断。
- 新增冰川玻璃和纯净玻璃主题：前者使用低色层轻磨砂，后者保持高通透的文字与图标界面；透明主题的控件图标和色块在浅色背景下仍保持可见。
- 主题体系调整为古典墨玉、曜石金、明亮白、星夜蓝、冰川玻璃、纯净玻璃六套配色。旧的绿色主题选择自动迁移到古典墨玉。
- 星夜蓝改用整面不重复的非规则 CSS 星点，窗口放大后不再呈现平铺周期。
- 音量控制改为继承主题颜色的 SVG 图标；标题栏色块、停止按钮、缩放提示和播放器边界视觉同步优化。
- 测试：**146 项**（offscreen 37 + background 61 + content 21 + logger 8 + theme 19）全部通过。
