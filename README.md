...
合集检测和导入同样由 `background.js` 完成：页面端只接收标题、数量和目标歌单等摘要，完整条目缓存在 Service Worker 中并一次写入本地存储。这样可以避免大型合集数据在页面、iframe 与后台之间重复传输；B站站内切页时，旧页面的迟到响应也不会覆盖当前页面状态。
...
安装完成后，打开任意普通网页，点击右下角的音符按钮即可展开播放器。在 B站视频页面展开面板后，可以使用顶部的“＋加入”保存当前视频；检测到合集或多 P 时，还可以使用“全部加入”批量导入。完整操作说明见 [USER_GUIDE.md](USER_GUIDE.md)。
