...
v1.8.4 拖拽改 window 级指针监听 + 松手即本地重排（修复"放不下/顺序不变"），
封面预览改为向右上方放大（避开左边框）。单元测试共 32 项全部通过。待用户在浏览器验证：
1. **按住封面拖动可排序，松手即落位**，刷新后顺序保留（不再卡半透明、不再还原）
2. 拖拽过程中封面**不会**触发悬停放大
3. 封面悬停约 2 秒后向**右上方**弹出 3 倍大图（不被左边框裁剪），移开即收回
